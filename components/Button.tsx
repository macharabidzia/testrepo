import React from "react";

interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: "primary" | "secondary" | "danger";
  isLoading?: boolean;
  size?: "sm" | "md" | "lg";
}

const baseStyles =
  "inline-flex items-center justify-center rounded-md shadow-sm transition-all duration-150 ease-in-out " +
  "focus:outline-none focus:ring-2 focus:ring-offset-2 " +
  "disabled:opacity-60 disabled:cursor-not-allowed disabled:shadow-none " +
  "active:translate-y-px active:shadow-sm";

const sizeStyles = {
  sm: "px-3 py-1.5 text-sm",
  md: "px-4 py-2 text-base",
  lg: "px-6 py-3 text-lg",
};

const variants = {
  primary:
    "bg-blue-500 text-white border border-transparent " +
    "hover:bg-blue-600 focus:ring-blue-500",
  secondary:
    "bg-white text-gray-700 border border-gray-300 " +
    "hover:bg-gray-100 focus:ring-gray-400",
  danger:
    "bg-red-500 text-white border border-transparent " +
    "hover:bg-red-600 focus:ring-red-500",
};

const loadingStyles = "relative pointer-events-none";

const Button: React.FC<ButtonProps> = ({
  variant = "primary",
  size = "md",
  className = "",
  children,
  disabled,
  isLoading,
  ...props
}) => {
  const isDisabled = disabled || isLoading;
  const spinnerColorClass =
    variant === "secondary" ? "text-gray-700" : "text-white";

  return (
    <button
      className={`${baseStyles} ${sizeStyles[size]} ${variants[variant]} ${
        isLoading ? loadingStyles : ""
      } ${className}`}
      disabled={isDisabled}
      {...props}
    >
      {isLoading && (
        <span className="absolute inset-0 flex items-center justify-center">
          <svg
            className={`animate-spin h-5 w-5 ${spinnerColorClass}`}
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
          >
            <circle
              className="opacity-25"
              cx="12"
              cy="12"
              r="10"
              stroke="currentColor"
              strokeWidth="4"
            ></circle>
            <path
              className="opacity-75"
              fill="currentColor"
              d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"
            ></path>
          </svg>
        </span>
      )}
      <span className={`${isLoading ? "invisible" : "visible"} flex-grow`}>
        {children}
      </span>
    </button>
  );
};

export default Button;
